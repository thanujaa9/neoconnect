...
export default function CaseDetail() {
  ...
  const handleEditNote = async (noteId) => {
    await api.updateNote(id, noteId, editText);
    setEditingNote(null);
    setEditText('');
    fetchCase();
  };
  ...
  const canAssign = user?.role === 'admin' ||
    (user?.role === 'secretariat' &&
      (!caseData?.assignedBy || caseData?.assignedBy?._id === user?.id));
    ...
    <Layout user={user}>
      <div className="max-w-3xl">
        ...
        <div className={`bg-white border border-slate-200 ${severityBorder[caseData.severity]} rounded-xl p-6 mb-4`}>
          ...
          <h1 className="text-xl font-semibold text-slate-800 mb-1">{caseData.title}</h1>
          <p className="text-sm text-slate-500 mb-4">{caseData.department} · {caseData.category}</p>
                ...
                {editingNote === n._id ? (
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="text-xs bg-teal-600 text-white px-3 py-1.5 rounded-lg"
                    >
                      Save
                    </button>
                    <button
                      onClick={() => setEditingNote(null)}
                      className="text-xs border border-slate-300 text-slate-600 px-3 py-1.5 rounded-lg"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <p className="text-xs text-slate-400">{n.addedBy?.name} · {new Date(n.addedAt).toLocaleDateString()}</p>
                      {(user?.role === 'case_manager' || user?.role === 'secretariat' || user?.role === 'admin') && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => { setEditingNote(n._id); setEditText(n.text); }}
        ...
        {user?.role === 'case_manager' && caseData.status !== 'Resolved' && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 mb-4">
            ...
            <p className="text-xs text-slate-400 mb-4">Fill this when closing the case. Visible to secretariat.</p>
            <form onSubmit={handleResolve} className="flex flex-col gap-3">
              ...
              <button
                type="submit"
                className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 rounded-lg text-sm"
              >
